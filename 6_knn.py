...
WN_inds = np.zeros((n_actions,n_samples)).astype('int')
...
def add_tuple(t,s,a,r,sPrime):
    ...
    for act in range(n_actions):
        #replace row
        knn_inds,sim = kernel(sPrime,S[act,:mem_count[a]])
    ...
    WN_inds[a,mask] = W[a,mask].argmax(1)
...
for a in range(n_actions):
    for v in valid_inds:
        inds,vals = kernel(SPrime_view[v],S[a,:mem_count[a]])
        W[a,v,inds] = vals[inds]
'''main loop'''
cumr = 0
refresh = int(1e0)
